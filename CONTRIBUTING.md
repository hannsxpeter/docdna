...
- **What input defeats it?** Write the bypass down even if you are not closing it. `docdna_backfill.py --verify` resolves a `code` anchor and then accepts any number that appears anywhere in that file, not only beside the anchor. That is the honest description, and it belongs next to the check rather than in a reviewer's head.
- **Does it verify, or does it accept an attestation?** Anything the model supplies and the tool re-reads is verification. Anything the model supplies and the tool takes at face value is attestation. A `run:` citation is attestation: docdna is read-only, so nothing executes the command and the writer produces both the command and its output. Attestation is legitimate; calling it verification is not.
