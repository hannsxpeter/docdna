...
docdna stands alone. It needs Python 3.8 on a POSIX host, and its filesystem layer checks for descriptor-relative, no-follow primitives before using them. No third-party Python package or other tool is required. Windows is not supported because Python does not expose equivalent race-safe filesystem primitives there. If you also use [codedna](https://github.com/hannsxpeter/codedna), which fingerprints how a repo writes code, treat it as a separate project that happens to install and wire the same way: nothing here imports it, tests against it, or degrades without it.
